...
# python3 rib.py chosen_config_file.dat
...
output_prefix = "rib_out_" # default
...
# read init file
# init file contains an initialized set of bezier control points
x_init_l = []
...
# initialize the cell dimensions
ax = 0
ay = 0
...
while sweep_flag:
	...
	cc11 = cc.ConnectingCurve5(co1.curve[1].at(1), 		
							   ...
							   co1.curve[0].tan_at(0), 			x0[30],
							   co1.curve[0].curv_vec_at(0),     x0[31],
							   Consts)

	cc22 = cc.ConnectingCurve5(co2.curve[0].at(1)+tr_w, 
							   co2.curve[0].tan_at(1), 			x0[30],
							   co2.curve[0].curv_vec_at(1),   	x0[31],
							   co2.curve[1].at(0),      
	...
	def update(x, *args):
		...
		cc11.update(co1.curve[1].at(1),		 
					...
					co1.curve[0].at(0)+tr_w, 
					co1.curve[0].tan_at(0), 		x[30],
					co1.curve[0].curv_vec_at(0),    x[31])

		cc22.update(co2.curve[0].at(1)+tr_w, 
					co2.curve[0].tan_at(1), 		x[30],
					co2.curve[0].curv_vec_at(1),    x[31],
					co2.curve[1].at(0),		 
	...
	def reparam_curve_t(x, t):
		...
		if t_rem >= 2 and t_rem < 3:
			return (np.dot(matrix_power(f_z,int(reps)),co1.curve[0].at(t_rem%1)+tr_w)+ reps*tr_c)

		if t_rem >= 3 and t_rem < 4:
			return (np.dot(matrix_power(f_z,int(reps)),cc12.curve.at(t_rem%1)+tr_w) + reps*tr_c)

		if t_rem >= 4 and t_rem < 5:
			return (np.dot(matrix_power(f_z,int(reps)),co2.curve[0].at(t_rem%1)+tr_w)+ reps*tr_c)
	...
	def contact_energy(x, print_to_file=False):
		...
		for idx1 in range(0,len(curve_points_mid)):
			...
			# sweep over nearby points and calculate compression enegry density
			for n in range(-3,4):
				for m in range(-2,3):
					for idx2 in range(0,len(curve_points_mid)):
						pt2 = np.dot(matrix_power(f_z,m),curve_points_mid[idx2]) + m*tr_c + n*tr_w
